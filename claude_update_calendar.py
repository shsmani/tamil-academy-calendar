...
def main():
    ...
    if not merged:
        ...
        idx = raw_text.upper().find("IMPORTANT DATES")
        if idx == -1:
            print("DEBUG: 'IMPORTANT DATES' not found in raw text at all.", file=sys.stderr)
        else:
            snippet = raw_text[idx:idx + 1500]
            print("DEBUG: raw text starting at 'IMPORTANT DATES' (next 1500 chars):", file=sys.stderr)
            print(repr(snippet), file=sys.stderr)
        sys.exit(1)
